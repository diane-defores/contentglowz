...
| `api/services/email_source_service.py` | `shipflow_data/technical/lab/architecture.md` | Per-user IMAP metadata, encrypted app-password lookup, validation state, and managed 6-hour ingestion job setup | Any email-source credential, folder, schedule, or validation contract change |
| `api/routers/settings_integrations.py` | `shipflow_data/technical/lab/architecture.md` | User-managed integration endpoints including OpenRouter and email source | Any integration route, credential response, or settings contract change |
| `api/routers/projects*.py` / `api/services/project*` | `shipflow_data/technical/lab/backend-runtime-and-product-apis.md` | Project selection, project CRUD, repository analysis, and project-scoped intelligence behavior | Any project selection, project CRUD, intelligence ingestion, or repository analysis contract change |
| `api/routers/video_timelines.py` / `api/models/video_timeline.py` / `api/migrations/005_video_timelines.sql` | `shipflow_data/technical/lab/backend-runtime-and-product-apis.md` | Video timeline draft/version/preview/final gates, render-safe assets, Remotion worker boundary, and artifact URL rules | Any timeline, render job, artifact, asset eligibility, or migration behavior change |
| `api/routers/assets*.py` / project asset services | `shipflow_data/technical/lab/backend-runtime-and-product-apis.md` | Project asset library routes, usage links, tombstone/restore, storage descriptors, and selection safety | Any project asset route, asset eligibility, storage descriptor, or tombstone behavior change |
| `api/routers/images*.py` / image generation services | `shipflow_data/technical/lab/backend-runtime-and-product-apis.md` | FLUX image generation, references, Bunny persistence, and project asset registration | Any image generation provider, reference, persistence, or asset-registration change |
| `api/routers/search_console*.py` / Search Console services | `shipflow_data/technical/lab/backend-runtime-and-product-apis.md` | Google Search Console OAuth setup, encrypted tokens, callback, and response redaction | Any GSC OAuth, token storage, callback, or Search Console response behavior change |
| `scheduler/scheduler_service.py` / `api/routers/idea_pool.py` / `agents/sources/ingest.py` | `shipflow_data/technical/lab/architecture.md` | Recurring Idea Pool ingestion jobs and newsletter inbox source ingestion | Any source ingestion, scheduler cadence, ownership, or archive behavior change |
| `agents/newsletter/tools/imap_tools.py` | `shipflow_data/technical/lab/architecture.md` | IMAP reader, folder validation, newsletter detection, processed-email archiving | Any IMAP credential, folder, detection, or archive behavior change |
