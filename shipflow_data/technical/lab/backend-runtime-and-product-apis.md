...
depends_on:
  ...
  - shipflow_data/technical/lab/architecture.md
  - shipflow_data/technical/worker/architecture.md
supersedes: []
...
- Cross-check worker render behavior with `shipflow_data/technical/worker/architecture.md`.
- Update this file when a README-local backend contract would otherwise be reintroduced.

## Maintenance Rule

Keep durable backend API and runtime contracts here or in a narrower `shipflow_data/technical/lab/*` module. Local `lab/README.md` stays a facade only.
