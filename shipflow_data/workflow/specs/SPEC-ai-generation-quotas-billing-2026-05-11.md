...
depends_on:
  ...
  - artifact: "shipflow_data/workflow/specs/contentflow_app/PRD-lifetime-deal-early-bird-payg.md"
...
evidence:
  ...
  - "contentflow_lab/status/cost_tracker.py already persists estimated external API costs by project, job, job_type, pipeline, mode, provider, and time range."
  - "contentflow_lab/api/services/job_store.py persists async jobs in Turso but does not store user_id/org_id or quota reservation data."
  - "contentflow_app/lib/data/services/api_service.dart already maps structured API error envelopes with code, kind, provider, settingsPath, and retryable."
  - "contentflow_app/lib/presentation/screens/settings/integrations_screen.dart already mentions BYOK versus ContentFlow-managed credits in AI runtime copy."
  - "contentflow_site/src/components/Pricing.astro currently states that all plans include AI generation costs and no hidden fees, which may conflict with BYOK/PAYG and future managed-credit packaging."
...
- Existing BYOK/product framing: `shipflow_data/workflow/specs/contentflow_app/PRD-lifetime-deal-early-bird-payg.md`.
- Existing BYOK enforcement spec: `shipflow_data/workflow/specs/contentflow_lab/SPEC-strict-byok-llm-app-visible-ai.md`.
- Existing async persistence: `contentflow_lab/api/services/job_store.py`.
- Existing provider error-normalization pattern: `contentflow_lab/api/routers/publish.py`.
- Existing auth dependency: `contentflow_lab/api/dependencies/auth.py`.
- Existing project ownership helpers in `contentflow_lab/api/dependencies/ownership.py` and project store patterns.
- Existing cost table and helpers: `contentflow_lab/status/cost_tracker.py` and `contentflow_lab/api/migrations/004_status_lifecycle.sql`.
- Existing Flutter API error envelope parsing: `contentflow_app/lib/data/services/api_service.dart`.
- Existing Riverpod provider wiring: `contentflow_app/lib/providers/providers.dart`.
- Existing app AI runtime/BYOK copy: `contentflow_app/lib/presentation/screens/settings/integrations_screen.dart`.
- Existing marketing pricing copy: `contentflow_site/src/components/Pricing.astro`.
- Project language guidelines: `contentflow_lab/shipflow_data/technical/guidelines.md`, `contentflow_app/shipflow_data/technical/guidelines.md`, and `contentflow_site/shipflow_data/technical/guidelines.md`.
...
- `contentflow_lab/api/services/job_store.py`: current jobs table lacks user/project indexes for quota reservation and owner-scoped queries. The future implementation may need a schema extension or a separate `ai_generation_jobs`/`usage_reservations` table instead of overloading `data`.
- `contentflow_lab/status/cost_tracker.py`: existing `api_cost_log` is useful for operational cost summaries but is too DataForSEO-shaped for quota enforcement. The new ledger should either extend it carefully or create a dedicated `ai_usage_ledger` while continuing to feed cost summaries.
- `contentflow_lab/api/routers/images.py`: Flux generation must call the quota service before provider submission and reconcile after completion.
- `contentflow_lab/api/services/flux_image_generation.py`: should return normalized actual-cost metadata when BFL exposes it.
- `contentflow_lab/agents/images/cdn_manager.py` and Bunny tools: upload/storage/optimizer usage may need cost events or estimates, especially for large uploads and generated variants.
...
- `contentflow_app/lib/data/services/api_service.dart`: should add typed usage/quota models and preserve structured error mapping.
- `contentflow_app/lib/providers/providers.dart`: should expose current quota/credit state, generation preflight state, and stale/refresh behavior through Riverpod.
- `contentflow_app/lib/presentation/screens/settings/integrations_screen.dart`: existing AI runtime copy already distinguishes BYOK and managed credits, but it needs real usage state and actions.
- `contentflow_site/src/components/Pricing.astro`: current "All plans include AI generation costs" copy is a product risk and must be revised before managed provider quotas are marketed.
...
- Update `contentflow_site/src/components/Pricing.astro` before launch of any managed-credit promise; current copy conflicts with the BYOK/PAYG strategy and future quota controls.
- Update `contentflow_lab` environment/setup docs with required provider cost config, usage ledger migration, admin override env/roles, and reconciliation job behavior.
...
- [ ] Task 1: Define the AI usage domain models
  - File: `contentflow_lab/api/models/ai_usage.py`
...
- [ ] Task 2: Create durable ledger and entitlement store
  - File: `contentflow_lab/api/services/ai_usage_store.py`
...
- [ ] Task 3: Add atomic reservation and reconciliation service
  - File: `contentflow_lab/api/services/ai_usage_service.py`
...
- [ ] Task 4: Define configurable action policies without choosing pricing
  - File: `contentflow_lab/api/services/ai_usage_policies.py`
...
- [ ] Task 8: Extend job metadata for owner-scoped quota operations
  - File: `contentflow_lab/api/services/job_store.py`
...
- [ ] Task 9: Add usage/quota API routes
  - File: `contentflow_lab/api/routers/ai_usage.py`
...
- [ ] Task 10: Add admin/ops adjustment and override flow
  - File: `contentflow_lab/api/routers/admin_ai_usage.py`
...
- [ ] Task 13: Add app UI contract for generation state and quota errors
  - File: `contentflow_app/lib/presentation/screens/settings/integrations_screen.dart`
...
- [ ] Task 14: Update marketing and support copy
  - File: `contentflow_site/src/components/Pricing.astro`
...
- Read these files first when implementing: `contentflow_lab/status/cost_tracker.py`, `contentflow_lab/api/services/job_store.py`, `contentflow_lab/api/dependencies/auth.py`, `contentflow_app/lib/data/services/api_service.dart`, and the current Flux implementation files.
