...
`AGENT.md` is the canonical technical and operational guidance file for `contentglowz_lab`.
`AGENTS.md` is kept only as a compatibility symlink for tools that still look up the plural filename.

## Purpose

This document defines the technical documentation baseline for `contentglowz_lab`.
...
This file provides guidance for AI agents working in the contentglowz codebase. It documents commands, patterns, conventions, and gotchas specific to this multi-component automation system.

---

## Operator Boundary — Do Not Cross

- `/home/claude/contentglowz/contentglowz_lab_deploy` is an operator-controlled deployment copy. Agents must not read, edit, test from, diff against, or use that directory as context.
...
- Work in `contentglowz_lab` only unless the user explicitly changes this project boundary.
...
# CONTENTGLOWZ_LIVE_TEST_BASE_URL is reachable during pytest collection.
```
...
- Production database URL: `libsql://contentglowz-<your-db>.aws-eu-west-1.turso.io`.
...
**Rule of thumb**
- Check current schema with Turso CLI first, for example `turso db shell <your-prod-turso-db> ".schema"` or targeted `PRAGMA table_info(...)` queries.
...
# Initial setup
doppler login
doppler setup              # Select project: contentglowz, config: dev
...
# Setup (one-time)
doppler login
doppler setup  # Select: contentglowz / dev
...
The repository uses `main` as the active branch.
...
- `AGENT.md` - Detailed agent specifications, workflows, quality metrics (this file)
...
# Optional
APP_URL=https://contentglowz.com        # For OpenRouter headers
```
...
**Quick test:**
```bash
cd /home/claude/contentglowz/contentglowz_lab
python -c "
