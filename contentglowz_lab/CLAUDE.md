...
`contentglowz_lab` is the production-oriented backend platform for the ContentGlowz ecosystem.

It hosts:

- a FastAPI runtime used by `contentglowz_app`,
...
- production_url: https://api.contentglowz.com
...
- Backend reliability changes must preserve compatibility for authenticated flows consumed by `contentglowz_app`.
...
- `contentglowz_app` — Flutter application and web shell.
- `contentglowz_site` — marketing/auth entrypoint.

## Forbidden Paths

- `/home/claude/contentglowz/contentglowz_lab_deploy` is an operator-controlled deployment copy of `contentglowz_lab`.
...
- Work in `contentglowz_lab` only; the user decides when and how deployment copies are updated.
