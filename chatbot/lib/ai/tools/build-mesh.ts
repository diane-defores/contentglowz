...
export const buildMeshTool = tool({
  description: "Build a new topical mesh structure from scratch for SEO content planning. Creates a pillar page with supporting cluster content.",
  inputSchema: z.object({
    mainTopic: z.string().describe("The main topic for the pillar page"),
    subtopics: z.array(z.string()).describe("List of subtopics for cluster pages"),
    businessGoals: z.array(z.string()).optional().describe("Business goals like 'rank', 'convert', 'educate'"),
    targetPages: z.number().optional().describe("Target number of pages in the mesh"),
