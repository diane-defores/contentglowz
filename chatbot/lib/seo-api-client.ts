...
const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://bizflowz-api.onrender.com';
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError') {
          ...
          throw new Error('Analysis is taking longer than expected. The repository may be large or complex. Please try again later.');
        }
  ...
  async analyzeInternalLinking(
    ...
    return this.request<{
      ...
      body: JSON.stringify({
        ...
        business_objectives: options?.businessObjectives || ['leads', 'demos', 'sales'],
      }),
    });
  }
