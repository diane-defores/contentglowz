...
test.describe("Internal Linking Card", () => {
	...
	test("should display default state initially", async ({ page }) => {
		await expect(
			page.getByText("No internal linking data available"),
		).toBeVisible();
		await expect(
			page.getByRole("button", { name: "Analyze Linking" }),
		).toBeVisible();
	});
	...
	test("should trigger analysis and update UI", async ({ page }) => {
		...
		await expect(
			page.getByText("Internal linking analysis complete!"),
		).toBeVisible();
		...
		await expect(
			page.getByRole("button", { name: "Apply Recommendations" }),
		).toBeVisible();
	});

	test("should open and save configuration", async ({ page }) => {
		await page
			.getByRole("button", { name: "Internal Linking" })
			.getByLabel("Settings")
			.click();
		await expect(
			page.getByText("Internal Linking Configuration"),
		).toBeVisible();
		...
		await expect(
			page.getByText("Internal linking strategy generated!"),
		).toBeVisible();
	...
	test("should apply recommendations", async ({ page }) => {
		...
		await expect(
			page.getByText("Internal links applied successfully!"),
		).toBeVisible();
	});
});
