(() => {
	...
	function captureElement(el) {
		...
		return loadHtml2Canvas()
			.then((html2canvas) => html2canvas(el, { useCORS: true }))
			.finally(() => {
	...
	function showScreenshotMenu(el, x, y) {
		...
		menu.style.cssText =
			"position:fixed;z-index:99999;background:#222;border-radius:8px;padding:4px 0;box-shadow:0 4px 16px rgba(0,0,0,.4);font-family:system-ui,sans-serif;font-size:14px;min-width:180px;";
		...
		var items = [
			{
				label: "Copier (clipboard)",
				action: () => {
					screenshotCopyClipboard(el);
				},
			},
			{
				label: "Télécharger PNG",
				action: () => {
					screenshotDownload(el);
				},
			},
			{
				label: "Upload + copier URL",
				action: () => {
					screenshotUpload(el);
				},
			},
		...
		items.forEach((item) => {
			...
			btn.style.cssText =
				"padding:8px 16px;color:#fff;cursor:pointer;transition:all 0.15s;";
	...
	function flashElement(div) {
		var color = div.style.outline
			? div.style.outline.match(/rgb\([^)]+\)/)[0]
			: "#FF0000";
	...
	function disableInspector() {
		...
		document.querySelectorAll(".buildflowz-outline").forEach((div) => {
	...
	function initToggleButton() {
		...
		toggleButton.style.cssText =
			"position:fixed;top:10px;right:10px;z-index:10000;background:#333;color:#fff;border:none;border-radius:50%;width:40px;height:40px;font-size:20px;cursor:pointer;box-shadow:0 2px 5px rgba(0,0,0,0.3);";
