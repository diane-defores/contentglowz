...
export function InternalLinkingCard({ repoUrl, authToken }: InternalLinkingCardProps) {
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <Progress value={analysis.seoOpportunities / analysis.totalOpportunities * 100} />
                <div className="text-xs text-muted-foreground">
                  {analysis.seoOpportunities} SEO • {analysis.conversionOpportunities} Conversion
                </div>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={analysis.linkingDensity >= 2 ? 'default' : 'destructive'}>
                    {analysis.linkingDensity.toFixed(1)}
                  </Badge>
                </div>
                <Progress value={Math.min(analysis.linkingDensity / 3 * 100, 100)} />
                <div className="text-xs text-muted-foreground">
                  Target: ≥2.0
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center">
                <div className="flex items-center justify-center gap-1">
                  ...
                  <span className="text-lg font-bold">+{analysis.authorityImpact}%</span>
                </div>
                <div className="text-xs text-muted-foreground">Authority Impact</div>
              </div>
              
              <div className="text-center">
                <div className="flex items-center justify-center gap-1">
                  <Zap className="h-4 w-4 text-green-500" />
                  <span className="text-lg font-bold">+{analysis.conversionImpact}%</span>
                </div>
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => analyzeLinking()} disabled={isLoading}>
                Re-analyze
              </Button>
              <Button size="sm" onClick={() => applyLinks()} disabled={isLoading}>
