...
export function useInternalLinking(repoUrl?: string, authToken?: string) {
	const [analysis, setAnalysis] = useState<InternalLinkingAnalysis | null>(
		null,
	);
	...
	const analyzeLinking = useCallback(
		async (
			...
			try {
				const result = (await seoApi.analyzeInternalLinking(url, {
					...
					personalizationLevel:
						options?.personalizationLevel || config.personalizationLevel,
					conversionFocus: options?.conversionFocus || config.conversionFocus,
					businessObjectives:
						options?.businessObjectives || config.businessObjectives,
				})) as any;
				...
				const normalizedResult: InternalLinkingAnalysis = {
					...result,
					totalOpportunities:
						result.total_opportunities || result.totalOpportunities || 0,
					seoOpportunities:
						result.seo_opportunities || result.seoOpportunities || 0,
					conversionOpportunities:
						result.conversion_opportunities ||
						result.conversionOpportunities ||
						0,
					authorityImpact:
						result.authority_impact || result.authorityImpact || 0,
					conversionImpact:
						result.conversion_impact || result.conversionImpact || 0,
					linkingDensity:
						result.linking_density ||
						result.linkingDensity ||
						result.summary?.linking_density ||
						0,
				};
	...
	const generateLinkingStrategy = useCallback(
		async (
			overrideRepoUrl?: string,
	...
	const applyLinks = useCallback(
		async (
			...
			try {
				...
				const message =
					err instanceof Error ? err.message : "Failed to apply links";
				setError(message);
				return null;
			} finally {
				setLoading(false);
	...
	const monitorPerformance = useCallback(
		async (
			...
			try {
				const result = await seoApi.monitorLinkingPerformance(
					url,
					timeframe,
					customStart,
					customEnd,
				...
				const message =
					err instanceof Error ? err.message : "Failed to monitor performance";
				setError(message);
				return null;
			} finally {
				setLoading(false);
