...
ContentFlow deploys its own Remotion worker container to Cloud Run. Do not use `@remotion/cloudrun` as the primary deployment path for this worker; the current Remotion docs mark that package alpha/not actively developed.

## Runtime Contract

- Cloud Run service: private HTTP service running `contentflow_remotion_worker`.
- Durable artifacts: private Google Cloud Storage bucket.
- Public auth boundary: `contentflow_lab` only.
- Playback: `contentflow_lab` signs short-lived GCS URLs after ownership checks.
...
- `CONTENTFLOW_RENDER_STORAGE=gcs`: production storage mode.
...
Local development can keep `CONTENTFLOW_RENDER_STORAGE=local` or omit it. Production must not fall back to local disk.
...
```bash
gcloud artifacts repositories create contentflow --repository-format=docker --location=europe-west1
gcloud builds submit contentflow_remotion_worker \
  --tag europe-west1-docker.pkg.dev/PROJECT_ID/contentflow/remotion-worker:latest
gcloud run deploy contentflow-remotion-worker \
  --image europe-west1-docker.pkg.dev/PROJECT_ID/contentflow/remotion-worker:latest \
  --region europe-west1 \
  --service-account contentflow-remotion-worker@PROJECT_ID.iam.gserviceaccount.com \
  --no-allow-unauthenticated \
  --memory 4Gi \
  --cpu 2 \
  --timeout 900 \
  --concurrency 1 \
  --set-env-vars CONTENTFLOW_RENDER_STORAGE=gcs,GCS_RENDER_BUCKET=BUCKET_NAME,GCS_RENDER_PREFIX=renders
```
