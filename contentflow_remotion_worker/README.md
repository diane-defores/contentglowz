# ContentFlow Remotion Worker

Internal worker service for local Remotion renders used by `contentflow_lab`.
...
- `CONTENTFLOW_RENDER_DIR` (required in shared deployments, defaults to `./renders`)
- `CONTENTFLOW_RENDER_STORAGE` (optional, `local` or `gcs`; default `local`)
- `GCS_RENDER_BUCKET` (required when `CONTENTFLOW_RENDER_STORAGE=gcs`)
...
```bash
npx remotion render remotion/index.ts ContentFlowTimelineVideo /tmp/contentflow-timeline-smoke.mp4 \
  ...
  "inputProps": {
    ...
    "clips": [
      ...
      },
      {
        ...
        "text": "Unified ContentFlow timeline"
      },
      {
        "id": "hero-image",
        "track_id": "visual-main",
...
- `ContentFlowTimelineVideo` supports `vertical_9_16` (`1080x1920`) and `landscape_16_9` (`1920x1080`) at `30fps`.
- Composition metadata is derived from `inputProps.format` via Remotion `calculateMetadata` (duration up to `5400` frames).
- Production timeline renders should receive server-resolved `assets[asset_id].render_url` values from `contentflow_lab`; missing media asset URLs render a neutral visual placeholder instead of crashing during local composition development.
...
- Set `CONTENTFLOW_RENDER_STORAGE=gcs` and `GCS_RENDER_BUCKET`.
...
- `contentflow_lab` owns signed playback URLs after user ownership checks.
