...
describe("render storage", () => {
  ...
  it("builds safe GCS object names", () => {
    assert.equal(
      buildGcsObjectName("job_1", "preview", "contentflow/renders"),
      "contentflow/renders/previews/job_1.mp4",
    );
  ...
  it("fails closed when GCS storage mode has no bucket", () => {
    const previousMode = process.env.CONTENTFLOW_RENDER_STORAGE;
    const previousBucket = process.env.GCS_RENDER_BUCKET;
    process.env.CONTENTFLOW_RENDER_STORAGE = "gcs";
    delete process.env.GCS_RENDER_BUCKET;
    try {
      assert.throws(() => readRenderStorageConfig(), /GCS_RENDER_BUCKET is required/);
    } finally {
      if (previousMode === undefined) {
        delete process.env.CONTENTFLOW_RENDER_STORAGE;
      } else {
        process.env.CONTENTFLOW_RENDER_STORAGE = previousMode;
      }
