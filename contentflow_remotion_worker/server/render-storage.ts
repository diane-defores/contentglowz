...
export function resolveRenderRootDir(): string {
  const configured = process.env.CONTENTFLOW_RENDER_DIR?.trim();
  if (configured) {
...
function normalizeStorageProvider(value: string | undefined): RenderStorageProvider {
  ...
  throw new Error("CONTENTFLOW_RENDER_STORAGE must be local or gcs");
}
...
export function readRenderStorageConfig(): RenderStorageConfig {
  const provider = normalizeStorageProvider(process.env.CONTENTFLOW_RENDER_STORAGE);
  ...
  if (!gcsBucket) {
    throw new Error("GCS_RENDER_BUCKET is required when CONTENTFLOW_RENDER_STORAGE=gcs");
  }
