...
UNSUPPORTED_CONTENTFLOW_CHANNELS = {"wordpress", "ghost"}
...
def _normalize_platform(platform: str) -> str:
    ...
    if normalized in UNSUPPORTED_CONTENTFLOW_CHANNELS:
        raise HTTPException(
...
async def _ensure_zernio_profile(user_id: str, project_id: str) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp = await client.post(
            ...
            headers=_headers(),
            json={
                "name": f"ContentFlow project {project_id}",
                "description": "ContentFlow project-scoped publishing profile",
            },
        )
...
async def connect_callback(state: str = Query(...)):
    ...
    return {
        ...
        "platform": session["platform"],
        "accounts": accounts,
        "message": "Connection completed. You can return to ContentFlow Settings.",
    }
