...
- Added the offline sync V2 reference spec in `shipflow_data/workflow/specs/contentglowz_app/SPEC-offline-sync-v2.md` and aligned project documentation with the current degraded-mode behavior.
...
### Added
- Added a website-driven web auth handoff so `contentglowz_site` can authenticate with Clerk and open Flutter web through `/api/auth/web/exchange` on `/entry`.
...
### Fixed
- Corrected the generated Android entrypoint/package path to match the renamed `com.contentglowz.contentglowz_app` namespace.
...
- Spec document: `shipflow_data/workflow/specs/contentglowz_app/SPEC-content-pipeline-unification.md`.
