...
Flutter product app for the ContentGlowz content pipeline.
...
It is online-only in V1 and uses `contentglowz_lab` as the only public API; the Flutter
...
- Each ContentGlowz project maps to a server-persisted Zernio profile.
...
- `SENTRY_RELEASE` (optional)
  Sentry release name. Defaults to `contentglowz_app@BUILD_COMMIT_SHA` when omitted and a commit is available.
- `SENTRY_DIST` (optional)
...
- `shipflow_data/workflow/specs/contentglowz_app/`
...
- Offline sync reference: `shipflow_data/workflow/specs/contentglowz_app/SPEC-offline-sync-v2.md`
