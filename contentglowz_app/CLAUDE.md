...
`contentglowz_app` is the Flutter product application for ContentGlowz, handling authenticated user onboarding, content operations, workspace management, review flows, scheduling, and diagnostics.
...
- **Site handoff** URLs from `contentglowz_site` and mobile/web launcher (`APP_SITE_URL`, `APP_WEB_URL`)
...
- Related repos:
  - `contentglowz_site` (landing, auth handoff entrypoint, public docs)
  - `contentglowz_lab` (AI agents and backend services)
