...
Internal worker service for local Remotion renders used by `lab`.
...
- Production timeline renders should receive server-resolved `assets[asset_id].render_url` values from `lab`; missing media asset URLs render a neutral visual placeholder instead of crashing during local composition development.
...
- `lab` owns signed playback URLs after user ownership checks.
