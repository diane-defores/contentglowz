...
export const contentGlowzTimelinePropsSchema = z
  .object({
    composition_id: z.literal("ContentGlowzTimelineVideo"),
...
export type ContentGlowzTimelineProps = z.infer<typeof contentGlowzTimelinePropsSchema>;

type TimelineTrack = ContentGlowzTimelineProps["tracks"][number];
type TimelineClip = ContentGlowzTimelineProps["clips"][number];
type AssetDescriptor = ContentGlowzTimelineProps["assets"][string];
...
function renderBackgroundClip(clip: TimelineClip, assets: ContentGlowzTimelineProps["assets"]): ReactElement {
...
function renderImageClip(clip: TimelineClip, assets: ContentGlowzTimelineProps["assets"]): ReactElement {
  const asset = clip.asset_ref ? assets[clip.asset_ref] : undefined;
  const src = resolveAssetSrc(asset);
  if (!src) {
...
function renderVideoClip(clip: TimelineClip, assets: ContentGlowzTimelineProps["assets"]): ReactElement {
  const asset = clip.asset_ref ? assets[clip.asset_ref] : undefined;
  const src = resolveAssetSrc(asset);
  if (!src) {
...
function renderVisualClip(clip: TimelineClip, assets: ContentGlowzTimelineProps["assets"]): ReactElement | null {
  switch (clip.type) {
    case "background":
...
export const deriveTimelineMetadata = (inputProps: unknown) => {
  const parsed = contentGlowzTimelinePropsSchema.safeParse(inputProps);
  if (parsed.success) {
    return {
...
export const ContentGlowzTimelineVideo = ({
  tracks,
  clips,
  assets,
}: ContentGlowzTimelineProps) => {
