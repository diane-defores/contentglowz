...
- Do not add ShipGlowz governance frontmatter to `src/content/**` unless `src/content.config.ts` explicitly accepts those fields.
- If content needs ShipGlowz governance, place the governance artifact under `shipglowz_data/editorial/`, `shipglowz_data/technical/`, or `shipglowz_data/workflow/specs/`, then reference the runtime content path.
