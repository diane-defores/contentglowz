...
linked_systems:
  ...
  - "app Android MediaProjection native capture"
  - "app local capture storage"
  - "app capture/content asset linking"
evidence:
  - "shipglowz_data/workflow/specs/app/SPEC-android-device-screen-capture.md defines Android MediaProjection local-only PNG/MP4 capture."
  - "shipglowz_data/workflow/specs/app/SPEC-local-capture-assets-linked-to-content.md keeps media local and stores only asset metadata server-side."
  - "app/lib/data/services/device_capture_service.dart exposes takeScreenshot/startRecording through platform channels."
  - "app/android/app/src/main/kotlin/com/contentglowz/app/capture/* records actual screen pixels through MediaProjection surfaces."
  ...
  - "shipglowz_data/workflow/research/shared/android-privacy-screen-redaction-technologies.md validates Android-native ML Kit/MediaCodec/MediaMuxer/Media3 technology choices for privacy redaction."
depends_on:
  - "shipglowz_data/workflow/specs/app/SPEC-android-device-screen-capture.md"
...
- `shipglowz_data/workflow/specs/app/SPEC-android-device-screen-capture.md` - Confirms V1 is Android MediaProjection, local-only, with screenshot and recording outputs saved as PNG/MP4.
- `shipglowz_data/workflow/specs/app/SPEC-local-capture-assets-linked-to-content.md` - Confirms captures may be linked to content but raw files remain local in V1.
- `app/lib/data/services/device_capture_service.dart` - Shows Flutter only starts/stops native capture and receives completed asset metadata.
- `app/lib/presentation/screens/capture/capture_screen.dart` - Shows the current user workflow has capture controls, local history, share, discard, and content attachment.
- `app/android/app/src/main/kotlin/com/contentglowz/app/capture/*` - Shows native code captures rendered pixels through MediaProjection, VirtualDisplay, ImageReader, and MediaRecorder.
