...
owner: "Diane"
confidence: medium
user_story: "En tant que mainteneuse du site marketing ContentGlowz, je veux migrer site d'Astro 5 vers Astro 6, afin de garder un build supporté, sécurisé et déployable sans régression SEO ni rupture des pages de contenu."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipglowz_data/technical/site/architecture.md"
    artifact_version: "0.1.0"
    required_status: "draft"
supersedes: []
evidence:
...
## Title
Migrate site from Astro 5 to Astro 6
...
- Current working directory: `/home/claude/contentglowz/site`
...
## User Story
En tant que mainteneuse du site marketing ContentGlowz, je veux migrer `site` d'Astro 5 vers Astro 6, afin de garder un build supporté, sécurisé et déployable sans régression SEO, contenu, sitemap, pages marketing, ni routes de redirection vers l'application.

Actor: mainteneuse du site marketing ContentGlowz.

Trigger: décision de migration majeure Astro 5 -> 6 sur le repo `site`.
...
## Scope Out
- Migration des autres repos `app` et `lab`.
...
## Constraints
- Repo autorisé: `/home/claude/contentglowz/site` uniquement.
- Fichier de spec autorisé: `/home/claude/contentglowz/shipglowz_data/workflow/specs/site/SPEC-migrate-astro-v6.md`.
...
- `shipglowz_data/business/business.md@1.0.0`: promesse produit et cohérence business.
- `shipglowz_data/branding/branding.md@1.0.0`: ton, positionnement, microcopy.
- `shipglowz_data/technical/site/guidelines.md@0.1.0`: dette metadata car statut draft.
- `shipglowz_data/technical/site/architecture.md@0.1.0`: architecture site statique et routes à préserver.
...
- `shipglowz_data/technical/site/architecture.md`: update content collection architecture from `src/content/config.ts` to `src/content.config.ts` if present.
- `shipglowz_data/editorial/site/content-map.md`: update only if route paths or content folders change.
...
- `shipglowz_data/technical/site/guidelines.md` and `shipglowz_data/technical/site/architecture.md` are draft metadata, while `shipglowz_data/business/business.md`, `shipglowz_data/branding/branding.md`, `shipglowz_data/product/site/product.md`, `shipglowz_data/gtm/site/gtm.md`, and `shipglowz_data/editorial/site/content-map.md` are reviewed. `/sf-ready` should decide whether draft docs block readiness.
...
- [ ] Tâche 13 : Mettre à jour la documentation minimale
  - Fichier : `README.md`, `CLAUDE.md`, `shipglowz_data/technical/site/architecture.md`, `CHANGELOG.md`
  ...
  - Depends on : Tâche 12.
  - Validate with : revue de diff; `rg -n "Astro 5|src/content/config.ts|Node 20|node 20|legacy collections" README.md CLAUDE.md shipglowz_data/technical/site/architecture.md CHANGELOG.md`.
...
```bash
cd /home/claude/contentglowz/site
git status --short
...
|----------|-------|-------|--------|--------|-----------|
| 2026-04-29 | sf-start | gpt-5 | Implemented Astro 6 migration, updated docs, regenerated lockfile, and validated build/preview outputs | implemented | /sf-verify Migrate site from Astro 5 to Astro 6 |
...
| 2026-04-29 | sf-end | gpt-5 | Closed Astro 6 migration session, aligned trackers, and updated changelog | closed | /sf-ship Migrate site from Astro 5 to Astro 6 |
...
| 2026-05-03 | sf-start | gpt-5 | Implemented post-verify cleanup: pinned Vercel npm commands, removed tracked venv from Git scope, documented development mode, and fixed stale architecture note | implemented | /sf-verify Migrate site from Astro 5 to Astro 6 |
...
| 2026-05-03 | sf-ship | gpt-5 | Shipped post-cleanup fixes for npm/Vercel alignment, repo hygiene, dev-mode docs, stale docs, changelog, and trackers | shipped | /sf-prod site |
