...
depends_on: []
supersedes: []
superseded_by: "shipglowz_data/workflow/specs/lab/SPEC-dual-mode-ai-runtime-all-providers.md"
evidence: []
next_step: "Superseded by shipglowz_data/workflow/specs/lab/SPEC-dual-mode-ai-runtime-all-providers.md"
---
# Title
Strict BYOK LLM For App-Visible AI Actions

## Status
Superseded by `shipglowz_data/workflow/specs/lab/SPEC-dual-mode-ai-runtime-all-providers.md`.
...
- `POST /api/personas/draft` already uses `UserProviderCredential` through `lab/api/services/user_llm_service.py`, but the rest of the visible AI flows do not.
...
- Changing demo-mode behavior in `app`
...
## Links & Consequences
- `lab/api/services/user_llm_service.py` becomes the single backend entrypoint for building:
  - OpenAI/OpenRouter SDK clients
  - `CrewAI` `LLM` instances
  - standardized runtime errors for missing/invalid credentials
- `lab/api/dependencies.py` and `lab/api/dependencies/agents.py` cannot keep request-agnostic singleton LLM agents for app-visible research flows. Those caches must be removed or bypassed for BYOK routes.
- `lab/api/models/psychology.py` must be aligned to current Flutter payloads, or the BYOK migration will still leave visible screens broken even if LLM resolution is correct.
- `app/lib/data/services/api_service.dart` must absorb polling for `ritual` and `angles`, because backend already models them as async jobs and the screens currently expect final data.
- `lab/api/routers/newsletter.py` must move off in-memory `_jobs` and onto `job_store` with `user_id`, or BYOK will still leave an owner-scope hole in a visible screen.
- `lab/api/routers/research.py` must accept the app's current conceptual inputs (`target_url`, `competitors`, `keywords`) instead of relying on a backend-only keyword contract.
- `lab/agents/seo/seo_crew.py`, `agents/newsletter/newsletter_crew.py`, `agents/short/short_crew.py`, and `agents/social/social_crew.py` currently perform nested status/content bookkeeping. Once `dispatch-pipeline` becomes the single outer orchestrator, those nested writes must be disabled in pipeline mode.
- `app/lib/presentation/screens/settings/settings_screen.dart` must stop describing OpenRouter as a persona-draft-only requirement.
...
- [ ] Task 1: Expand the user-scoped LLM runtime service
  - File: `lab/api/services/user_llm_service.py`
...
- [ ] Task 2: Normalize the psychology request contracts to match current Flutter usage
  - File: `lab/api/models/psychology.py`
...
- [ ] Task 4: Refactor psychology agents to accept explicit LLM injection
  - File: `lab/agents/psychology/creator_psychologist.py`
...
- [ ] Task 6: Standardize missing-key UX in the app and update OpenRouter copy
  - File: `app/lib/presentation/screens/settings/settings_screen.dart`
...
- [ ] Task 7: Remove implicit env-based LLM resolution from short/social/newsletter/article crews
  - File: `lab/agents/short/short_crew.py`
...
- [ ] Task 8: Repair the article pipeline foundation while moving it to BYOK
  - File: `lab/agents/seo/seo_crew.py`
...
- [ ] Task 9: Migrate newsletter generation to BYOK plus persisted owner-scoped jobs
  - File: `lab/api/routers/newsletter.py`
...
- [ ] Task 10: Make newsletter config readiness user-aware without converting non-LLM dependencies to BYOK
  - File: `lab/agents/newsletter/config/newsletter_config.py`
...
- [ ] Task 11: Replace cached singleton research agents with request-scoped BYOK construction
  - File: `lab/api/routers/research.py`
...
- [ ] Task 12: Align the research API contract with the current app screen
  - File: `lab/api/models/research.py`
...
- [ ] Task 13: Remove unsafe legacy fallback from the Angles screen content-generation path
  - File: `app/lib/presentation/screens/angles/angles_screen.dart`
...
- [ ] Task 14: Add regression coverage proving there is no env fallback on app-visible routes
  - File: `lab/tests/test_psychology_auth_jobs.py`
...
- [ ] Task 15: Add a small Flutter test surface for BYOK error detection
  - File: `app/test/core/byok_guard_test.dart`
...
- [ ] Task 16: Make the Research screen explicit about keyword requirements
  - File: `app/lib/presentation/screens/research/research_screen.dart`
...
- [ ] CA 4: Given a valid user OpenRouter credential, when the user runs `Ritual`, then `app` receives a final `NarrativeSynthesisResult` after internal polling and the backend job remains owner-scoped.
- [ ] CA 5: Given a valid user OpenRouter credential, when the user runs `Generate Angles`, then `app` receives final angle data after internal polling and no global LLM env var is needed.
