---
artifact: brand_context
metadata_schema_version: "1.0"
artifact_version: "1.0.0"
project: contentglowz
created: "2026-04-25"
updated: "2026-06-28"
status: reviewed
source_skill: sf-docs
scope: brand
owner: "Diane"
confidence: medium
risk_level: medium
security_impact: none
docs_impact: yes
brand_voice: clear, practical, direct
trust_posture: transparency over hype, explicit constraints, automation with optional review
evidence:
  - shipglowz_data/business/business.md
  - README.md
depends_on:
  - shipglowz_data/business/business.md@1.0.0
supersedes:
  - shipglowz_data/business/app/branding.md
  - shipglowz_data/business/site/branding.md
  - shipglowz_data/business/lab/branding.md
next_review: "2026-07-28"
next_step: /sf-docs audit shipglowz_data/branding/branding.md
---
# Branding Guide

## Brand

- **Brand name**: ContentGlowz
- **Category**: AI-first content operations product
- **Primary message**: "Ready-made content, optional edits, fast publish"
- **Primary promise**: Deliver ready-made branded content that teams can publish fast or adjust when needed.

## Brand Positioning

ContentGlowz is positioned as an AI-first execution system that combines:

- idea intake,
- source asset intake,
- automatic draft assembly,
- structured curation,
- optional review and edits,
- scheduling control,
- resilient continuity when backend services are unavailable.

The brand promise is shared across all surfaces:

- `site` explains and converts,
- `app` executes the operator workflow,
- `lab` and `worker` support reliable delivery,
- no surface may create a competing promise.

## Voice and Tone

- Clear, practical, and direct.
- Professional but not corporate.
- Concise with explicit status states and explicit limits.
- Encourage trust through transparency rather than hype.
- Avoid fake certainty or magical claims; describe the default as automatic draft creation with optional review.

## Language

- Primary product language is English unless a surface explicitly targets French content.
- When writing in French, use informal address ("tu") consistently.
- When writing in French, always keep proper French accents.
- Use short, explicit labels in navigation, status, and conversion surfaces.

## Visual and UX Language

- Functional-first with strong affordances and readable hierarchy.
- Status, retries, degraded mode, and trust copy are part of the brand system.
- Accessibility and feedback clarity are branding concerns, not only technical concerns.

## Terms to Use

- "ready to publish"
- "optional edits"
- "swipe to publish"
- "publish with intention"
- "degraded mode"
- "content workflow"
- "sync queue"
- "queued action"
- "bootstrap"
- "trust and continuity"

## Terms to Avoid

- "miracle"
- "set and forget"
- "no oversight required"
- vague AI claims implying perfect outcomes

## Experience Principles

- Keep users in control, even when the default path is automated.
- Make waiting states understandable in one sentence.
- Preserve user momentum with explicit recovery paths.
- Make uncertainty explicit and actionable.
