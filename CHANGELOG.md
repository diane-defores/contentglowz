...
### Removed
- Removed generated Flutter web build artifacts from Git tracking so Vercel owns `contentglowz_app/build/web` generation during deployment.
...
- Added baseline `shipflow_data/technical/` governance for `contentglowz_app` and `contentglowz_site`.
...
- Migrated `contentglowz_site` tracking to Astro 6 completion and documented the site migration closure.
...
- Consolidated `contentglowz_lab` agent guidance into `AGENT.md` and kept `AGENTS.md` as the compatibility symlink.
