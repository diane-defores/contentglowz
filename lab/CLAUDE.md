...
- Backend reliability changes must preserve compatibility for authenticated flows consumed by `app`.
...
- `app` — Flutter application and web shell.
- `site` — marketing/auth entrypoint.

## Forbidden Paths

- `/home/claude/contentglowz/lab_deploy` is an operator-controlled deployment copy of `lab`.
...
- Work in `lab` only; the user decides when and how deployment copies are updated.
...
- `ENVIRONMENT_shipflow_data/technical/SETUP.md` for secrets and runtime configuration.
