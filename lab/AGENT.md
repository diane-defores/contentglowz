...
`AGENT.md` is the canonical technical and operational guidance file for `lab`.
`AGENTS.md` is kept only as a compatibility symlink for tools that still look up the plural filename.

## Purpose

This document defines the technical documentation baseline for `lab`.
...
- `/home/claude/contentglowz/lab_deploy` is an operator-controlled deployment copy. Agents must not read, edit, test from, diff against, or use that directory as context.
...
- Work in `lab` only unless the user explicitly changes this project boundary.
...
- `docs/GROQ_shipflow_data/technical/SETUP.md` - Groq API setup
...
**Quick test:**
```bash
cd /home/claude/contentglowz/lab
python -c "
