...
Backend platform for ContentGlowz, centered on FastAPI services and AI automation pipelines for content strategy, scheduling, analytics, and delivery support.

This repository hosts the product API used by:

- `app` (Flutter application),
- `site` (landing pages + auth handoff flow),
...
2. Configure secrets with Doppler or `.env` fallback:
   - `doppler login`
   - `doppler setup` (`contentglowz` project + `dev`)
...
The canonical video timeline lives in `lab`, not in Remotion or
...
renderer adapter behind `worker`.
...
- Required render env vars follow the worker contract:
  `REMOTION_WORKER_URL`, `REMOTION_WORKER_TOKEN`, `CONTENTGLOWZ_RENDER_DIR`, and
...
`lab` remains the authenticated boundary for video preview/final
...
`CONTENTGLOWZ_RENDER_DIR`. Production mode uses a private GCS bucket:
