...
- **app** — Flutter application (Web, iOS, Android)
- **lab** — FastAPI backend + AI agents (Python)

## Cross-Repo Contract Notes

- `site` is a public Astro surface and does not own backend schema.
- If a site change affects app handoff contract (`APP_WEB_URL`, `/sign-in`, `/sign-up`, `/launch`) or public capability claims, align docs in `app` and `lab` in the same session.
