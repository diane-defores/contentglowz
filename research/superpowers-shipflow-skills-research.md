---
artifact: research
project: "contentglowz / ShipFlow"
created: "2026-05-17"
updated: "2026-05-17"
status: reviewed
source_skill: sf-research
scope: "Superpowers Claude plugin and implications for improving ShipFlow skills"
confidence: high
risk_level: medium
security_impact: yes
docs_impact: yes
...
next_step: "/sf-spec Ameliorer les skills ShipFlow avec les patterns Superpowers: TDD de skills, debugging systematique, revue en deux temps, worktrees optionnels, et gates supply-chain"
---

# Research: Superpowers et amelioration du set de skills ShipFlow

> Generated 2026-05-17 - Sources: 13

## Executive Summary

Superpowers est pertinent pour ShipFlow, mais comme source de patterns, pas comme dependance a installer telle quelle. Ses points forts sont la discipline d'execution: brainstorming avant code, plan tres actionnable, TDD strict, debugging par cause racine, sous-agents avec revue, verification avant cloture, et support worktree. ShipFlow couvre deja une grande partie du lifecycle, mais peut gagner en fiabilite avec trois ajouts: un protocole TDD/RED-GREEN adapte aux skills, une doctrine de debugging plus stricte pour `sf-fix`/`sf-bug`, et une revue en deux temps pour les executions deleguees.
...
Le README met l'accent sur des workflows obligatoires, pas des conseils optionnels. Les skills les plus differenciants pour ShipFlow sont:
...
ShipFlow a deja des equivalents partiels:
...
L'ecart principal est moins structurel que comportemental: Superpowers formule des "iron laws" tres contraignantes. ShipFlow est plus riche en gouvernance, mais parfois moins mordant sur les preuves avant correction, les tests avant implementation et la revue post-tache.
...
**Cons**:
- Risque de conflit avec les conventions ShipFlow: chantiers, specs, reporting, model routing, topology, tracking.
...
**Best for**: experimentation personnelle hors runtime ShipFlow, pas pour le corpus ShipFlow canonique.

### Option 2: Copier des skills Superpowers dans ShipFlow
...
- Import direct de texte externe sans adaptation aux contrats ShipFlow.

**Best for**: spike local court, puis suppression ou integration propre.

### Option 3: Adapter les patterns dans les references ShipFlow

**Pros**:
- Preserve le lifecycle ShipFlow.
...
4. Formaliser une revue en deux temps dans la delegation ShipFlow: conformite au plan/spec, puis qualite code/risque/tests.
...
6. Garder les descriptions ShipFlow compactes. Les docs Claude Code disent que les descriptions servent a la selection et que le corps complet reste charge apres invocation; la spec Agent Skills recommande aussi progressive disclosure et `SKILL.md` sous 500 lignes.
...
Recommandation: ouvrir un chantier ShipFlow dedie, pas installer Superpowers directement.
...
- P3: worktrees optionnels. Utile mais pas aussi central, car Codex et ShipFlow ont deja des contraintes de dirty worktree et ownership.
...
- Installer Superpowers comme plugin parallele peut creer des instructions concurrentes avec ShipFlow, surtout sur delegation, plan, verification et ship.
...
Titre propose: Ameliorer les skills ShipFlow avec les patterns Superpowers
Raison: Les recommandations touchent plusieurs skills et references ShipFlow, avec decisions de lifecycle, tests, delegation, verification et securite supply-chain.
Severite: P2
Scope: ShipFlow skills corpus, references de lifecycle, bug/fix/test/start/verify/skill-build/delegation workflows.
Evidence:
- Superpowers expose un workflow TDD/debug/revue plus strict que les gates ShipFlow actuels.
- ShipFlow est sous le budget de decouverte, donc une integration ciblee est possible sans suppression de skills.
- Les sources securite 2026 montrent qu'une installation directe de skills tiers augmente la surface de risque.
Spec recommandee: /sf-spec Ameliorer les skills ShipFlow avec les patterns Superpowers: TDD de skills, debugging systematique, revue en deux temps, worktrees optionnels, et gates supply-chain
Prochaine etape: creer une spec ShipFlow dediee avant modification du corpus.

## Chantier

non trace: aucun chantier unique ContentGlowz ne porte cette recherche ShipFlow transverse.

Flux: sf-spec pending -> sf-ready pending -> sf-start pending -> sf-verify pending -> sf-end pending -> sf-ship pending
Reste a faire: creer une spec dediee si on decide d'integrer ces patterns dans ShipFlow.
Prochaine etape: /sf-spec Ameliorer les skills ShipFlow avec les patterns Superpowers: TDD de skills, debugging systematique, revue en deux temps, worktrees optionnels, et gates supply-chain
