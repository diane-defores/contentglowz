...
**ContentGlowz Site** — Marketing website built with Astro. Static site with blog, pricing, features, and SEO content.
...
- production_url: https://contentglowz.com
...
- **contentglowz_app** — Flutter application (Web, iOS, Android)
- **contentglowz_lab** — FastAPI backend + AI agents (Python)

## Cross-Repo Contract Notes

- `contentglowz_site` is a public Astro surface and does not own backend schema.
- If a site change affects app handoff contract (`APP_WEB_URL`, `/sign-in`, `/sign-up`, `/launch`) or public capability claims, align docs in `contentglowz_app` and `contentglowz_lab` in the same session.
