...
It is online-only in V1 and uses `lab` as the only public API; the Flutter
...
- `SENTRY_RELEASE` (optional)
  Sentry release name. Defaults to `app@BUILD_COMMIT_SHA` when omitted and a commit is available.
- `SENTRY_DIST` (optional)
