...
linked_systems:
  ...
  - "contentflow_app Android MediaProjection native capture"
  - "contentflow_app local capture storage"
  - "contentflow_app capture/content asset linking"
evidence:
  - "contentflow_app/specs/SPEC-android-device-screen-capture.md defines Android MediaProjection local-only PNG/MP4 capture."
  - "contentflow_app/specs/SPEC-local-capture-assets-linked-to-content.md keeps media local and stores only asset metadata server-side."
  - "contentflow_app/lib/data/services/device_capture_service.dart exposes takeScreenshot/startRecording through platform channels."
  - "contentflow_app/android/app/src/main/kotlin/com/contentflow/contentflow_app/capture/* records actual screen pixels through MediaProjection surfaces."
  ...
  - "research/android-privacy-screen-redaction-technologies.md validates Android-native ML Kit/MediaCodec/MediaMuxer/Media3 technology choices for privacy redaction."
depends_on:
  - "contentflow_app/specs/SPEC-android-device-screen-capture.md"
...
- `contentflow_app/specs/SPEC-android-device-screen-capture.md` - Confirms V1 is Android MediaProjection, local-only, with screenshot and recording outputs saved as PNG/MP4.
- `contentflow_app/specs/SPEC-local-capture-assets-linked-to-content.md` - Confirms captures may be linked to content but raw files remain local in V1.
- `contentflow_app/lib/data/services/device_capture_service.dart` - Shows Flutter only starts/stops native capture and receives completed asset metadata.
- `contentflow_app/lib/presentation/screens/capture/capture_screen.dart` - Shows the current user workflow has capture controls, local history, share, discard, and content attachment.
- `contentflow_app/android/app/src/main/kotlin/com/contentflow/contentflow_app/capture/*` - Shows native code captures rendered pixels through MediaProjection, VirtualDisplay, ImageReader, and MediaRecorder.
...
- [Android privacy redaction technology research](../../research/android-privacy-screen-redaction-technologies.md) - Created 2026-05-07 - Used to select ML Kit Text Recognition, MediaProjection, MediaCodec/MediaMuxer, Media3 Transformer, and optional AccessibilityService as the likely implementation stack.
